...
| `enable_local_backup` | Nightly gzipped tarball of `data/` + `logs/` into `smartboi_run/backups/`, keeping 14 dailies and 8 weeklies. **On by default.** The accumulated evidence and the forward record cannot be regenerated -- there is no replay path -- so this is the one failure in the system with no recovery. It lands in the mapped `/config` share, so it is visible over Samba and is included in a Home Assistant partial backup of `/config`. It is still the same physical host: copy one off the box periodically |
